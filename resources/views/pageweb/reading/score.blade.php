...
<head>
    ...
    <title>Score - Reading Quiz</title>
            ...
            <div class="card">
                <div class="card-body text-center">
                    <div class="card shadow border-0 mb-4 py-4 px-3"
                        ...
                        <h3 class="mb-4" style="color: #003766; font-size: 28px;">
                            Halo, {{ auth()->check() ? (auth()->user()->nama_lengkap ?? 'User') : 'User' }}</h3>
                        @if($scoreReading)
                            <p style="color: #0056b3; font-size: 18px; margin-bottom: 15px;">Skor Anda saat ini
                                adalah:</p>
                            <h4 class="font-weight-bold text-primary"
                                style="font-size: 48px; color: #007bff;">{{ $scoreReading->score }}</h4>
                            <p class="mt-4" style="color: #4e586e; font-size: 16px;">Apakah Anda ingin meningkatkan skor
                                Anda? Mulailah kuis lagi sekarang!</p>
                        ...
                        @else
                            <p class="text-muted" style="font-size: 16px;">Anda belum memiliki skor. Silakan kerjakan
                                kuis untuk mendapatkan skor pertama Anda!</p>
                            <div class="d-flex justify-content-center">
                                <a href="{{ route('pageweb.reading') }}" class="btn btn-primary me-2"
                                   style="width: 150px; background: linear-gradient(135deg, #007bff, #0056b3); color: #fff; border: none; transition: transform 0.2s;">Mulai
                                    Kuis</a>
                            </div>
                        @endif
                    </div>
